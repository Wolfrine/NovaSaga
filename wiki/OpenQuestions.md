# Unsettled Records

This page keeps open matters in one place so the other records can read cleanly.

## The Event

- Is the electricity collapse permanent, long-term, regional, or partly reversible?
- Does the Event follow a 3000-year cycle, or is that only an inherited fragment from older accounts?
- Is the cause cosmic, planetary, metaphysical, material, or a convergence of several forces?

## The First Companions

- What are the companions' names, ages, home region, social background, family ties, and inner flaws?
- Does the first group contain four boys, five boys, or another composition?
- Is one companion captured and corrupted by dark forces?

## Luminary Names and Orders

- Are Ascendants, Radiant Luminaries, and Luminaries separate groups, historical stages, titles, or variant names?
- Are city, village, field, and research Luminaries formal categories or rough archive classifications?

## Dark Beings

- Are dark beings corrupted humans, separate entities, or both?
- How much tragedy, guilt, or agency remains in corrupted forms?
- Can corrupted belief or misused reverence feed existing dark beings, draw them closer, create new ones, or only weaken communities?

## Cosmology

- Does disbelief literally weaken powers, or does it only force secrecy?
- How much scientific language should remain around aura, frequency, resonance, and energy?
- How exactly do disbelief, fear, reverence, and collective attention interact with the unseen order?

## Sacred Traditions

- Did Luminaries seed all religions, selected traditions, symbolic practices only, later guarded human traditions, or participate in mixed histories?
- Are gods symbolic containers, misunderstood Luminaries, higher entities, human inventions, or different things in different traditions?
- Is dogma degraded symbolic guidance, human corruption, dark influence, or a mixture?
- How much of the religion-Luminary relationship is known to modern Luminaries?
- How much should be revealed to the first companions during early training?

## Portals and Wider Worlds

- Do portals belong to the central record, later histories, or archive-only expansion?
- Do extraterrestrial radiants exist as allies, symbolic accounts, or discarded terminology?
- Do later histories extend beyond Earth?

## Source Evidence

- `raw/2026-06-02-legacy-foundation-curation.md`
- `raw/2026-06-02-luminaries-religion-context-guidance.md`
- `archive/legacy-before-restart-2026-06-02/Plot_outline.md`
- `archive/legacy-before-restart-2026-06-02/Plot_raw.md`
